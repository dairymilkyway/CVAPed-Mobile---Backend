...
const EXERCISE_SERVICE_URL = getServiceUrl('THERAPY_URL', 5002);

/**
 * GET /api/exercises/can-analyze
 * Check if user can perform gait analysis today
 */
router.get('/can-analyze/:userId', protect, async (req, res) => {
  try {
    const { userId } = req.params;

    if (req.user.role !== 'admin' && req.user.role !== 'therapist' && req.user._id.toString() !== userId) {
      return res.status(403).json({
        success: false,
        message: 'Not authorized to check this user\'s exercise status'
      });
    }
...
router.post('/recommend', protect, async (req, res) => {
  try {
    ...
    if (req.user.role !== 'admin' && req.user.role !== 'therapist' && req.user._id.toString() !== user_id) {
      return res.status(403).json({
        success: false,
        message: 'Not authorized to generate recommendations for this user'
      });
    }
